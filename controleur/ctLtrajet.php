...
switch($action){
		...
		case 'enregistrer': 
		{
				...
				{
				echo "Immatriculation non conforme";
				include('vues/trajets/formtrajet.php');
                break;
				}
		...
		case 'reservationcov':
				{   $cherchetrajet = ModelTrajet::chercherreserves($_GET['iduser'],$_GET['idcov'] );
					$idep = ModelTrajet::cherchedep($_GET['idcov']);
					$dep = $idep['Lieudep'];
					$arr = $idep['Lieuarr'];
					ModelTrajet::reservertrajets($_GET['iduser'],$_GET['idcov'],$dep,$arr, $_GET['cond'],$_GET['statut']);
					$listetrajet = ModelTrajet::mestrajetsreserves($_SESSION['id']);	
					include('vues/trajets/listetrajet.php');
					break;
				}
